...
void backpatchlabel(backpatch *head, unsigned label){
...
expr *relop_actions(expr *e, iopcode op, expr *arg1, expr *arg2, unsigned label, int line){

	e->truelist = pushbackpatchList(nextquadlabel());
	e->falselist = pushbackpatchList(nextquadlabel() + 1);
	emit(op, NULL, arg1, arg2, label, line);
...
void pushassistemit(iopcode iop, expr *result, expr *arg1, expr *arg2, unsigned label, unsigned line){
	assistquad *tmphead=assistquads;
	assistquad *p=malloc(sizeof(assistquad));
	p->op = iop;
	p->result = result;
	p->arg1 = arg1;
	p->arg2 = arg2;
...
assistquad* popassistemit(){
	
	
	if(assistquads!=NULL){
	assistquad *temp2 = assistquads;
	assistquads = assistquads->next;
	assistquad *returnval = temp2;
	//free(temp2);
	return returnval;}
	return NULL;
