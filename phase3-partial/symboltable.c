...
void PrintScopeList() {
	...
	while (tmp != NULL)
	{
		...
		while (tmpentry != NULL) {
			if (tmpentry->type == USERFUNC || tmpentry-> type == LIBFUNC) {
				printf("Key = %s  %s  line = %d  scope = %d  isActive = %d offset = scope_space = %s\n", tmpentry->value.funcVal->name, stringify(tmpentry->type), tmpentry->value.funcVal->line, tmpentry->value.funcVal->scope, tmpentry->isActive ,tmpentry->offset, stringify_scope_space(tmpentry->scope_space));
