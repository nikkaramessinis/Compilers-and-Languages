...
SymbolTableEntry *insert(bool isActive, char *name, int scope , int line, enum SymbolTableType type) {
	...
	if (type == LIBFUNC) {
		...
		//if (LookUpLibFunc(name, localFlag, line) == 1) {
			//return;
		//}
...
void PrintScopeList() {
	...
	while (tmp != NULL)
	{
		...
		while (tmpentry != NULL) {
			...
			{
				printf("Key = %s  %s  line = %d  scope = %d  isActive = %d\n", tmpentry->value.varVal->name, stringify(tmpentry->type), tmpentry->value.varVal->line, tmpentry->value.varVal->scope, tmpentry->isActive);
			}
			tmpentry = tmpentry->scopenext;
		}
		tmp = tmp->next;
