...
expr *add_front(expr* expression, expr *self){
 	
 	if(expression == NULL){
 		return NULL;
 	}
	expr *temp2 = expression->expr_head;

	if(expression->expr_head == NULL){
		expression->expr_head = self;
		self->next = NULL;
	}else{
		self->next = temp2;
		expression->expr_head = self;
...
void printelist(expr *head){
    
    expr * temp=head->expr_head;
    while(temp!=NULL){
       // printf("Label:%u\n",temp->type);
        temp=temp->next;
    }
...
expr *emit_iftableitem(expr * e, unsigned int scope,unsigned int line)
{
...
expr *member_item(expr * lvalue,char * name,unsigned int scope,unsigned int line)
{  
...
void reversedElist(expr *expression){

	expr* prev = NULL;
	if(expression==NULL){
		return;
	}
    expr* current = expression->expr_head;
    expr* next = NULL;
    while (current != NULL)
    {
        next  = current->next;  
    ...
    expression->expr_head = prev;

}

expr *make_call(expr *lvalue, expr *elist_t,unsigned int scope,unsigned int line){
	expr *tmp;
	expr *func = (expr *)emit_iftableitem(lvalue, scope, line);
	reversedElist(elist_t);
	
	if(elist_t==NULL){
		tmp=NULL;
	}else{
		tmp = elist_t->expr_head;
	}
	while(tmp != NULL){
...
expr *insertExpression(expr *expression,expr *toinsert){

	if(expression == NULL){
		return NULL;
	}
	expr *tmp = expression->expr_head;
	
	if(expression->expr_head == NULL){
	
		expression->expr_head = toinsert;
		expression->expr_head->next = NULL;
	}else{
		while(tmp->next != NULL){
			tmp = tmp->next;
		}
...
void printQuads(){
	...
	for(i = 0; i < currQuad; i++){
		...
		if(quads[i].result != NULL){
			if(quads[i].result->strConst!=NULL && (quads[i].result->type == CONSTSTRING_E || quads[i].result->type == NIL_E)){
		...
		if(quads[i].arg1 != NULL){
			if(quads[i].arg1->strConst!=NULL && (quads[i].arg1->type == CONSTSTRING_E || quads[i].arg1->type == NIL_E)){
		...
		if(quads[i].arg2 != NULL){
			if(quads[i].arg2->strConst!=NULL && (quads[i].arg2->type == CONSTSTRING_E || quads[i].arg2->type == NIL_E)){
