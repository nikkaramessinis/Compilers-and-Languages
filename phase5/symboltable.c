		...
		if (tmp == NULL) {
			...
			while (tmpentry != NULL) {
				if (tmpentry->type == USERFUNC) {
					//mini allagh
					if (strcmp(tmpentry->value.funcVal->name, name)==0 && tmpentry->isActive==1){
						return tmpentry;
					}

				} else {
					//mini allagh2 
					if (strcmp(tmpentry->value.varVal->name, name)==0 && tmpentry->isActive==1){
						return tmpentry;
					}
				}
				tmpentry = tmpentry->scopenext;
...
SymbolTableEntry *insert(bool isActive, char *name, int scope , int line, enum SymbolTableType type) {
	...
	if (type == LIBFUNC) {
		...
		temp->line = line;
		temp->scope = scope;
		//malloc(sizeof(expr)*len * sizeof(char));
		token = malloc(sizeof(SymbolTableEntry));
		token->scopenext = NULL;
		token->isActive = isActive;
		token->type = type;
...
unsigned global_variables(){
	...
	while (tmp != NULL)
	{
		...
		while (tmpentry != NULL) {
			if(tmpentry->scope_space== PROGRAM_VAR && (tmpentry->type==VAR_E || tmpentry->type==TABLEITEM_E)){
			total++;
		}
