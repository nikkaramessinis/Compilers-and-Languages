...
expr: assignexpr 		{ $$=$1;
	...
	| expr NOT_EQUAL expr 	{ 	printf("expr -> expr != expr\n");
								printf("STON KANONA TO $3 EXEI %s\n", $3->strConst);
								$$ = (expr*)newexpr(BOOLEXPR_E);
								$$->sym = (SymbolTableEntry *)new_temp(curr_scope, yylineno);
	...
	| lvalue INCREMENT 			{ 	printf("term -> lvalue++\n");
									if($1 != NULL){
										...
										if($1->type == TABLEITEM_E){
											...
											//allagi
											emit(TABLESETELEM, val,$1->index, $1,  -1, NULL);
										}else{
...
call: call '(' elist ')' 	{ printf("call -> call(elist)\n"); 
	...
	| lvalue callsuffix 			{ printf("call -> lvalue callsuffix\n"); 
										if($1 != NULL){
											if($2->method){
												expr *self = $1;
												expr *member=(expr *)member_item(self, $2->name, curr_scope, yylineno);
												$1 = (expr *)emit_iftableitem(member, curr_scope, yylineno);
												$2->elist_t = (expr *)add_front($2->elist_t,self);

											}
											$$ = (expr *)make_call($1, $2->elist_t,curr_scope, yylineno);
										}
									}
...
elist:  expr 					{ printf("elist	-> expr\n"); $$->expr_head = NULL; $$ = (expr *)insertExpression($$,$1); printelist($$);}
		| elist  ',' expr 		{ printf("elist	-> elist , expr*\n"); $$= (expr *)insertExpression($$,$3);printelist($$);}
		|						{ printf("elist	-> empty\n"); $$ = NULL;}
		;
			...
			{ 
				int ret_val = LookUp($2,curr_scope,yylineno,1);
					...
					//	
						...
						$$->functionjump=(backpatch *)pushbackpatchList(nextquadlabel());
						emit(JUMP, NULL, NULL, NULL, -1, yylineno);
					...
					else if(ret_val == 1){//enterscopespace();
...
funcdef: funcprefix funcargs funcbody	{	printf("funcdef -> function id (idlist) block\n");
											...
											patchlabel($1->functionjump->label,nextquadlabel());
										}
